...
AutoSequelize.prototype.build = function (callback) {
  ...
  function mapTable(table, _callback) {
    self.queryInterface
      .describeTable(table, self.options.schema)
  ...
  function mapForeignKeys(table, fn) {
    ...
    var sql = self.dialect.getForeignKeysQuery(
      table,
      self.sequelize.config.database,
    );

    var commentSql = self.dialect.getTableFiledComment
      ? self.dialect.getTableFiledComment(table, self.sequelize.config.database)
      : null;
    self.sequelize
      .query(sql, {
    ...
    function assignColumnDetails(ref) {
      ...
      if (
        _.isFunction(self.dialect.isPrimaryKey) &&
        self.dialect.isPrimaryKey(ref)
      )
        ref.isPrimaryKey = true;

      if (
        _.isFunction(self.dialect.isSerialKey) &&
        self.dialect.isSerialKey(ref)
      )
        ref.isSerialKey = true;

      self.foreignKeys[table] = self.foreignKeys[table] || {};
      self.foreignKeys[table][ref.source_column] = _.assign(
        {},
        self.foreignKeys[table][ref.source_column],
        ref,
      );
    }
  }
  ...
  function generateText(err) {
    ...
    async.each(
      ...
      function (table, _callback) {
        ...
        text[table] +=
          spaces + "return sequelize.define('" + tableName + "', {\n";

        _.each(fields, function (field, i) {
          var additional = self.options.additional;
          if (
            additional &&
            additional.timestamps !== undefined &&
          ...
          // Find foreign key
          var foreignKey =
            self.foreignKeys[table] && self.foreignKeys[table][field]
              ? self.foreignKeys[table][field]
              : null;
          ...
          // ENUMs for postgres...
          if (
            self.tables[table][field].type === 'USER-DEFINED' &&
          ...
          var isUnique =
            ...
            self.tables[table][field].foreignKey.isUnique;
          ...
          _.each(fieldAttr, function (attr, x) {
            ...
            if (attr === 'foreignKey') {
              ...
              if (_.isString(defaultVal)) {
                ...
                if (
                  ...
                  if (_.endsWith(defaultVal, '()')) {
                    val_text =
                      "sequelize.fn('" + defaultVal.replace(/\(\)$/, '') + "')";
                  } else if (
                    _.includes(
                      [
                        'current_timestamp',
              ...
              if (defaultVal === null || defaultVal === undefined) {
                ...
                text[table] +=
                  spaces + spaces + spaces + attr + ': ' + val_text;
              }
            } else if (
              attr === 'type' &&
              self.tables[table][field][attr].indexOf('ENUM') === 0
            ) {
              text[table] +=
                spaces +
                spaces +
              ...
              var val =
                quoteWrapper + self.tables[table][field][attr] + quoteWrapper;
        ...
        var hasadditional =
          _.isObject(self.options.additional) &&
          _.keys(self.options.additional).length > 0;
        ...
        function addAdditionalOption(value, key) {
          if (key === 'name') {
            ...
            text[table] +=
              spaces + spaces + spaces + "singular: '" + table + "',\n";
            text[table] +=
              spaces + spaces + spaces + "plural: '" + table + "'\n";
            text[table] += spaces + spaces + '},\n';
          } else {
