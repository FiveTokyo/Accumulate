...
class Index {
    ...
    generate() {
        ...
        for(var m in models) {
            ...
            routerPageCon.push(`${space}router.get('/${apiPrefx}/${replaceEntity.__tablename__}/query', controller.${replaceEntity.__Tablename__}.searchByApi);`);

        }
        let fileCon = this.template.replace(/__PAGE_ROUTERS__/g, routerPageCon.join('\n'));
        fs.writeFileSync(`${this.savePath}/api.js`, fileCon);
    }
}
